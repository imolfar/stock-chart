...
(function() {
  angular.module('StockChart', ['chart.js']).service('ChartApi', function($http) {
    return {
      getLast30Days: function(code, callback) {
        return $http.get('chart_api/getLast30Days/' + code).success(function(res) {
          return callback(res);
        }).error(function(res) {
  ...
  }).controller('StockChartController', function($scope, ChartApi) {
    return $scope.loadChart = function() {
      if ($scope.code === '') {
        return;
      }
      return ChartApi.getLast30Days($scope.code, function(res) {
        var data, labels, prevMonth;
        if (res === false) {
          $scope.message = 'No data available at the time';
        }
        if (res.status !== 'Ok') {
        ...
        data = [];
        prevMonth = false;
        res.result.forEach(function(dayPrice) {
          var date, day;
          ...
          labels.push(day);
          prevMonth = date[1];
          return data.push(dayPrice.price.toFixed(2));
        });
